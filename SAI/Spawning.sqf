...
SAI_SPAWN_LOGISTICS = {
	...
	private _spawns = SAI_SPAWN_WEST_LOG;
	if (_side == SAI_EAST) then {_spawns = SAI_SPAWN_EAST_LOG};
	if (count _spawns > 0) then {
		private _select = _spawns select floor random count _spawns;
...
for "_i" from 1 to (SAI_CFG_SPAWNS_EAST) do {
...
[SAI_WEST, SAI_POS_REAR_WEST] call SAI_SPAWN_LOGISTICS;
[SAI_EAST, SAI_POS_REAR_EAST] call SAI_SPAWN_LOGISTICS;
